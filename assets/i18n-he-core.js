(() => {
  ...
  Object.assign(state.pageMeta, {
    'core-getting-started': 'תחילת עבודה עם SmartPT Console: רישוי, מצב המוצרים, הרשאות וכניסה לפורטלים.',
    'core-portal-overview': 'סקירת SmartPT Console, כרטיסי המוצרים, עדכונים, פעילות אחרונה והגדרות.',
    ...
    'core-settings-overview': 'הגדרות הרשאה, משך הפעלה, איפוס 2FA ומידע על הרישיון.',
    'core-shared-2fa-reset': 'איפוס רישום 2FA למשתמש והשלמת רישום מחדש בכניסה הבאה.'
  });

  Object.assign(state.pages, {
    'core-getting-started': page('תחילת עבודה עם SmartPT Console', 'SmartPT Core', `
      <p class="lead">השתמשו במסוף כדי לבדוק את הרישיון, לפתוח את AD Control ואת JIT Access ולנהל הרשאות כניסה למסוף.</p>
      ...
      <h2>הגדרה ראשונית</h2><ol><li>היכנסו ל-<strong>SmartPT Console</strong>.</li><li>ודאו שמצב הרישיון <strong>ACTIVE</strong>.</li><li>בדקו שכרטיסי AD Control ו-JIT Access תקינים.</li><li>פתחו <strong>Settings</strong> והוסיפו קבוצת AD ייעודית למנהלי המסוף.</li><li>הוסיפו קבוצות צפייה רק למשתמשים הזקוקים לגישה לקריאה בלבד.</li><li>בדקו את משך ההפעלה ואת זמן הניתוק מחוסר פעילות.</li></ol>
      <h2>תוצאה צפויה</h2><p>מנהלים יכולים לשנות הגדרות ולעדכן מוצרים. צופים יכולים לראות מידע ולפתוח פורטל מוצר רק אם יש להם הרשאה נפרדת באותו מוצר.</p>
      <h2>בדיקה</h2><p>היכנסו פעם אחת עם חשבון מנהל ופעם נוספת עם חשבון צופה והשוו את הפעולות הזמינות.</p>
    `),

    'core-portal-overview': page('סקירת SmartPT Console', 'SmartPT Core', `
      <p class="lead">המסוף מציג את מצב SmartPT Core, המוצרים המותקנים, פעילות אחרונה והגדרות משותפות.</p>
      ...
      <div class="table-wrap"><table><thead><tr><th>אזור</th><th>למי זמין</th><th>שימוש</th></tr></thead><tbody>
        ...
        <tr><td><strong>Settings</strong></td><td>מנהלים</td><td>הרשאות, זמני הפעלה, איפוס 2FA ורישוי.</td></tr>
      </tbody></table></div>
      <h2>עדכון מוצר</h2>
      ...
      <h2>פעילות אחרונה</h2>${shot('./docs/core/screenshots/recent-activity-highlight.png', 'אזור Recent Activity במסוף', 'השתמשו ב-Recent Activity לבדיקת פעולות אחרונות.')}
    `),

    'core-license-status': page('מצב רישיון ומוצרים', 'SmartPT Core', `
      <p class="lead">בדקו שרישיון Core פעיל ושכל פורטל מוצר זמין.</p>
      ...
      <h2>בדיקת המוצרים</h2>${shot('./docs/core/screenshots/product-status-highlight.png', 'מצב מוצרי SmartPT', 'כל כרטיס מוצר צריך להציג מצב תקין וקישור לפורטל.')}
    ...
    'core-access-model': page('מודל הרשאות למסוף', 'SmartPT Core', `
      <p class="lead">SmartPT Console משתמש בקבוצות Active Directory כדי להעניק הרשאת מנהל או צפייה.</p>
      ...
      <h2>תוצאה צפויה</h2><p>חברי הקבוצה רואים פעולות מנהל במסוף. הרשאות AD Control ו-JIT נשארות נפרדות.</p>
    `),

    'core-settings-overview': page('הגדרות SmartPT Console', 'SmartPT Core', `
      <p class="lead">פתחו <strong>Settings</strong> כדי לנהל הרשאות למסוף, זמני הפעלה, איפוס 2FA ומידע על הרישיון.</p>
      ...
      <div class="table-wrap"><table><thead><tr><th>אזור</th><th>שימוש</th></tr></thead><tbody>
        ...
        <tr><td><strong>Maximum session lifetime</strong></td><td>משך ההפעלה המרבי.</td></tr>
      ...
      ${shot('./docs/core/screenshots/settings-rbac-session-highlight.png', 'הגדרות RBAC והפעלה', 'בדקו את קבוצות הגישה ואת מגבלות ההפעלה לפני ייצור.')}
    ...
    'core-shared-2fa-reset': page('איפוס רישום 2FA', 'SmartPT Core', `
      <p class="lead">אפסו את רישום ה-2FA כאשר משתמש החליף מכשיר, איבד את אמצעי האימות או צריך להירשם מחדש.</p>
      ...
      <h2>תוצאה צפויה</h2><p>המשתמש נדרש להשלים רישום 2FA מחדש. הפעולה אינה מקצה רישיון מוצר או תפקיד.</p>
    `)
  });
})();
